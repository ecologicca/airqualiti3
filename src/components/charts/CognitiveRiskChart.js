...
const CognitiveRiskChart = ({ userPreferences, airQualitySettings }) => {
  ...
  const calculateBaseCognitiveRisk = (pm25Value) => {
    ...
    console.log(`PM2.5: ${pm25Value} -> Base Risk: ${clampedRisk}`);
    return clampedRisk;
  };
    ...
    try {
      ...
      const baseData = sortedData.map(item => {
        ...
        return {
          ...
          risk: calculateBaseCognitiveRisk(pm25),
          pm25: pm25
        };
      });
  ...
  if (loading) return <div>Loading cognitive risk data...</div>;
